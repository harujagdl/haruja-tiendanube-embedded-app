...
const finalizeDictionary = (itemsByCategory) => {
  ...
  CATEGORY_KEYS.forEach((category) => {
    ...
    const byCode = items.reduce((acc, item) => {
      acc[item.codigo] = item;
      return acc;
    }, {});

    result[category] = { items, byCode };
  });
...
const main = async () => {
  ...
  if (isDryRun) {
    ...
    CATEGORY_KEYS.forEach((category) => {
      console.log(`[dry] ${category}: ${dictionary[category].items.length} items`);
    });
    return;
  }
  ...
  for (const category of CATEGORY_KEYS) {
    const payload = dictionary[category];
    await db.collection('diccionario').doc(category).set(payload, { merge: true });
    console.log(`${category}: ${payload.items.length} items subidos`);
  }
};
