...
const parseWorkbook = (workbook) => {
  ...
  workbook.SheetNames.forEach((sheetName) => {
    ...
    for (let rowIndex = startIndex; rowIndex < rows.length; rowIndex += 1) {
      ...
      // ✅ KEY FINAL (MISMA QUE EL FRONTEND)
...
const main = async () => {
  ...
  if (invalidRows.length) {
    ...
    invalidRows.slice(0, 10).forEach((invalid) => {
      console.log(`- [${invalid.sheet} row ${invalid.row}] ${invalid.code} -> ${invalid.reason}`);
    });
  }
  ...
  for (const [key, entry] of counters.entries()) {
    ...
    // ✅ Guardar AMBOS para compatibilidad: el frontend usa value hoy
    batch.set(
      counterRef,
      {
        value: entry.value,
